...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Ensure AudioContext / Tone.js is resumed from a user gesture.
  // Many browsers block AudioContext until a user gesture — add a one-time
  // listener to resume on first click or touchstart so playback buttons work.
  (function ensureAudioResume() {
    async function resumeAudio() {
      try {
        if (window.Tone && typeof window.Tone.start === 'function') {
          await window.Tone.start();
          console.info('[app] AudioContext resumed via user gesture');
        } else if (window.audioContext && typeof window.audioContext.resume === 'function') {
          await window.audioContext.resume();
          console.info('[app] audioContext.resume() called via user gesture');
        }
      } catch (err) {
        console.warn('[app] resume audio failed:', err);
      }
    }

    const handler = () => { resumeAudio().catch(() => {}); document.removeEventListener('click', handler); document.removeEventListener('touchstart', handler); };
